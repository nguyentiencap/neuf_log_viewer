...
class NEUFLogService {
  ...
  /**
   * Check if temp context filter table exists
   * @param {DatabaseWrapper} db - Database wrapper instance
   * @returns {boolean}
   */
  _tempContextTableExists(db) {
    const result = db.prepare(
      `SELECT name FROM sqlite_temp_master WHERE type='table' AND name='temp_context_filter'`
    ).get();
    return result !== null;
  }
  ...
  async scanLogs(folderPath) {
    ...
    // Parse and index logs with batch callback
    const totalLogs = await scannerService.parseAndIndexLogFiles(
      logFolderPath,
      (batch) => insertMany(batch)
    );
  ...
  async getFilterOptions(folderPath, filters = {}, limitedOptions = true) {
    const { db, databaseService } = await this.loadDatabase(folderPath);

    // Determine which table to use for filter options
    let tableName = 'logs';
    
    // Check if temp context filter table exists (only if search with context)
    if (filters.search && filters.contextLines > 0) {
      if (this._tempContextTableExists(db)) {
        tableName = 'temp_context_filter';
      }
    }

    const filterOptions = databaseService.getFilterOptions(filters, tableName, limitedOptions);
