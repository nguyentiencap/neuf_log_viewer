...
class NEUFLogService {
  ...
  async scanLogs(folderPath) {
    ...
    while (true) {
      ...
      for (const raw of rawPage) {
        ...
        if (logObj) {
          // Use time_bucket already computed in Phase 1
          logObj.timeBucket = raw.time_bucket;
          batch.push(logObj);
          totalLogs++;
   ...
   * Each step reads from inputTable, applies its filters, and writes results to outputTable.
   *
   * @param {string} folderPath - Path to log folder
   * @param {Array<{filters: Object, inputTable: string, outputTable: string}>} steps
   * @param {Object} options - Pagination options (page, pageSize)
   * @returns {Object} { success, logs, total, page, pageSize, totalPages, stepCounts }
   */
  async filterLogs(folderPath, steps, options = {}) {
    ...
    for (const step of steps) {
      const inputTable = step.inputTable;
      const outputTable = step.outputTable;
      const filters = step.filters || {};

      const { count } = databaseService.executeFilterStep(filters, inputTable, outputTable);
      stepCounts.push(count);
    }

    // Determine final table (last step's outputTable)
    const lastOutputTable = steps.length > 0 ? steps[steps.length - 1].outputTable : 'logs';

    // Read all rows from final table and paginate
    const allRows = databaseService.db.prepare(
      `SELECT * FROM ${lastOutputTable} ORDER BY timestamp ASC`
    ).all();

    const total = allRows.length;
    const start = (page - 1) * pageSize;
    const logs = allRows.slice(start, start + pageSize);
   ...
   * @param {string} inputTable - Table to query options from (default: 'logs')
   * @returns {Object} Available filter options
   */
  async getFilterOptions(folderPath, filters = {}, limitedOptions = true, inputTable = 'logs') {
    const { databaseService } = await this.loadDatabase(folderPath);

    const filterOptions = databaseService.getFilterOptions(filters, inputTable, limitedOptions);
