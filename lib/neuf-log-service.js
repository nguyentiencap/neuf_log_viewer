...
class NEUFLogService {
   ...
   *
   * When contextLines > 0 (context-lines mode): materializes a temp table named
   * 'filter_<md5(filtersKey)>'. The temp table is cached — on repeated calls with
   * identical filters only the paging query runs, skipping DROP/CREATE/INSERT.
   *
   * When contextLines is 0 (simple filter mode): queries the source table directly
   * with a WHERE clause. No temp table is created or required.
   *
   * @param {string} folderPath - Path to log folder
   * @param {string} inputTable - Input table to filter
   ...
   */
  async filterLogs(folderPath, inputTable, filters, options = {}) {
    ...
    this.logger(`🔍 Filtering logs with page: ${page}, pageSize: ${pageSize}, sourceTable: ${sourceTable}, filters: ${JSON.stringify(normalizedFilters)}`);

    const needsTempTable = !!(normalizedFilters.search && normalizedFilters.contextLines > 0);

    let total, logs, outputTable;

    if (needsTempTable) {
      // Context-lines mode: use a cached temp table so paginated requests don't
      // rebuild the table on every call.
      const filtersKey = JSON.stringify(normalizedFilters);
      outputTable = this._getFilterOutputTable(filtersKey);
      databaseService.executeFilterStep(normalizedFilters, sourceTable, outputTable);
      total = this._getOutputTableCount(databaseService, outputTable);
      logs = databaseService.db.prepare(
        `SELECT * FROM ${outputTable} ORDER BY timestamp ASC LIMIT ? OFFSET ?`
      ).all(pageSize, offset);
    } else {
      // Simple filter mode: query the source table directly — no temp table overhead.
      const { where, params } = databaseService.buildWhereClause(normalizedFilters);
      const countRow = databaseService.db.prepare(
        `SELECT COUNT(*) as count FROM ${sourceTable} ${where}`
      ).get(...params);
      total = countRow ? countRow.count : 0;
      logs = databaseService.db.prepare(
        `SELECT * FROM ${sourceTable} ${where} ORDER BY timestamp ASC LIMIT ? OFFSET ?`
      ).all(...params, pageSize, offset);
      // No temp table: return the source table so getFilterOptions can query it
      // (the caller should pass the active filters back to getFilterOptions).
      outputTable = sourceTable;
    }
   ...
   * Reads filters.preset (string or array), resolves from preset suggestions,
   ...
   * @param {string|null} inputTable - Optional filter input table name from client
   * @returns {Promise<void>}
   */
  async applyPreset(folderPath, inputTable = null, filters) {
    ...
    const optionsResult = await this.getFilterOptions(folderPath, inputTable);
    if (!optionsResult.success) return;

    const suggestions = PresetService.getSuggestions(optionsResult.data, filters);
    const suggestionMap = new Map(suggestions.map(suggestion => [suggestion.id, suggestion]));

    for (const presetId of presetIds) {
      const suggestion = suggestionMap.get(presetId);
      if (!suggestion) {
        this.logger(`⚠️  Preset not found: "${presetId}"`);
        continue;
      }

      Object.assign(filters, suggestion.filters);
    }
  }

  /**
   * Get preset suggestions based on current filter context.
   * Resolves filter options from DB, then delegates suggestion generation to PresetService.
   * @param {string} folderPath - Path to log folder
   * @param {string|null} inputTable - Optional filter output table name from client
   * @param {Object} filters - Currently applied filters (already normalized, preset applied)
   * @returns {Promise<Object>} { success, suggestions: [{ id, label, description }] }
   */
  async getPresetSuggestions(folderPath, inputTable = null, filters = {}) {
    this.logger(`💡 Getting preset suggestions for filters: ${JSON.stringify(filters)}`);
    const result = await this.getFilterOptions(folderPath, inputTable);

    if (!result.success) {
      return { success: false, suggestions: [] };
    }

    const suggestions = PresetService.getSuggestions(result.data, filters);
   ...
   *
   * Two usage modes:
   * 1. Temp-table mode: pass a valid `inputTable` (returned by filterLogs in context-lines
   *    mode). The options are read from the pre-filtered temp table — no WHERE clause needed.
   * 2. Direct-filter mode: no valid `inputTable` (simple filter mode skips temp tables).
   *    Pass the active `filters` so a WHERE clause is applied directly on the source table.
   *
   * @param {string} folderPath - Path to log folder
   * @param {string|null} inputTable - Optional filter output table name from client
   * @param {boolean} limitedOptions - Limit thread_name and components to top 20 (default: true)
   * @param {Object|null} filters - Active filters to apply when no temp table is available
   * @returns {Object} Available filter options
   */
  async getFilterOptions(folderPath, inputTable = null, limitedOptions = true, filters = null) {
    const { databaseService } = await this.loadDatabase(folderPath);

    const sourceTable = this._isValidTable(inputTable) ? inputTable : 'logs';

    // When the caller has no pre-filtered temp table, scope options to the active filters.
    // Spread filters to avoid mutating the caller's object — normalizeFilters deletes keys in place.
    const effectiveFilters = !this._isValidTable(inputTable) && filters
      ? this._normalizeTimeBucketFilters(this.normalizeFilters({ ...filters }))
      : null;

    this.logger(`📋 Getting filter options, sourceTable: ${sourceTable}, limitedOptions: ${limitedOptions}, usingFilters: ${!!effectiveFilters}`);

    const filterOptions = databaseService.getFilterOptions(sourceTable, limitedOptions, effectiveFilters);
