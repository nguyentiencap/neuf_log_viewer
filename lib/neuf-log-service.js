...
class NEUFLogService {
  ...
  async filterLogs(folderPath, steps, options = {}) {
    ...
    for (let i = 0; i < steps.length; i++) {
      ...
      databaseService.executeFilterStep(filters, inputTable, outputTable);
    }

    // Determine final table (last step's outputTable)
    const lastOutputTable = steps.length > 0 ? NEUFLogService.getOutputTableByStep(steps.length - 1) : 'logs';
