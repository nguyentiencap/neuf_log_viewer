...
class PresetService {
  ...
  static getPresetSuggestions(filterOptions = {}) {
    ...
    if (components.length >= 5) {
      ...
      const anotherComponents = ['HistoricalSessionDataPersistenceHandler', 'HistoricalSessionManager',
        'ASEndpointTesterCallerFactory', 'EndpointMonitorService ', 'EndpointHealthMonitorService', 'EndpointHealthController', 'OMEndpointTesterCallerFactory', 'EQEndpointTesterCallerFactory'
      ];
      for (const comp of anotherComponents) {
        if (!topComponents.includes(comp)) {
