...
class DatabaseService {
   ...
   * When a pre-filtered temp table is available pass it as tableName; when querying
   * the raw logs table directly pass the active filters so a WHERE clause is applied.
   ...
   * @param {Object|null} filters - Optional filters to apply as WHERE clause (used when no temp table)
   */
  getFilterOptions(tableName = 'logs', limitedOptions = true, filters = null) {
    // Build WHERE clause from filters when provided (no pre-filtered temp table)
    ...
    const getOptions = (column, orderBy, limit = null) => {
      ...
      const columnCheck = `${column} IS NOT NULL`;
      const fullWhere = where
        ? `${where} AND ${columnCheck}`
        : `WHERE ${columnCheck}`;
      const query = `SELECT ${column}, COUNT(*) as count FROM ${tableName} ${fullWhere} GROUP BY ${column} ORDER BY ${orderBy}${limitClause}`;
      return this.db.prepare(query).all(...params);
    };

    const totalWhere = where || 'WHERE 1=1';
    const totalResult = this.db.prepare(`SELECT COUNT(*) as total FROM ${tableName} ${totalWhere}`).get(...params);
    const totalLogs = totalResult ? totalResult.total : 0;

    const getTimeBuckets = () => {
      const timeBucketCheck = `time_bucket IS NOT NULL`;
      const timeBucketWhere = where
        ? `${where} AND ${timeBucketCheck}`
        : `WHERE ${timeBucketCheck}`;
      const query = `
  ...
  executeFilterStep(filters, inputTable, outputTable) {
    if (inputTable !== outputTable) {
      // Check if this temp table was already built for an identical filter set.
      // The output table name is a hash of the filters, so the same name always
      // means the same data — no need to rebuild it for paginated requests.
      const existsRow = this.db.prepare(
        `SELECT name FROM sqlite_temp_master WHERE type='table' AND name=?`
      ).get(outputTable);

      if (!existsRow) {
        // Create output temp table
        this.db.exec(`
  ...
  /**
   * Drop a specific temp table by name (used to invalidate a cached filter table).
   * @param {string} tableName - Temp table name to drop
   */
