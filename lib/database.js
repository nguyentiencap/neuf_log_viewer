...
class DatabaseService {
  ...
  /**
   * Build a SQL subquery that returns the IDs of all logs in the context range.
   * Uses a CTE-style subquery: for each log matching search+filters, include
   * all logs with id between matchId-n and matchId+n.
   * @param {Object} filters - Full filters including search and contextLines
   * @returns {Object} { subquery: string, params: Array } - subquery string (no trailing semicolon) and bound params
   */
  _buildContextIdSubquery(filters) {
    const ctxN = parseInt(filters.contextLines, 10);
    if (!Number.isInteger(ctxN) || ctxN <= 0) {
      throw new Error('Invalid contextLines value for _buildContextIdSubquery');
    }
    ...
    // avoids the need for a second bound parameter in every nested subquery.
    const subquery = `
    ...
    return { subquery, params: matchParams };
  }
  ...
  searchWithContextLines(filters, options) {
    ...
    // Validate orderBy: allow only safe identifier+direction patterns
    // e.g. "timestamp ASC", "id DESC", "timestamp ASC, id DESC"
    const safeOrderByPattern = /^[a-zA-Z_][a-zA-Z0-9_]*(\s+(ASC|DESC))?(\s*,\s*[a-zA-Z_][a-zA-Z0-9_]*(\s+(ASC|DESC))?)*$/i;
    const safeOrderBy = safeOrderByPattern.test(orderBy) ? orderBy : 'timestamp ASC';
  ...
  getFilterOptions(filters, tableName = 'logs', limitedOptions = true) {
    ...
    // When context is active, build a subquery that returns the IDs of all
    // logs in the context result set.  This subquery is appended to every
    // per-field query so the counts reflect the context window, not the full
    // table.  Using a subquery means no temp table is needed and the result is
    // correct even when filter_option is called in a separate HTTP request.
    let contextSubquery = null;
    let contextParams = [];

    if (hasContextFilter) {
      const { subquery, params } = this._buildContextIdSubquery(filters);
      contextSubquery = subquery;
    ...
    const getOptions = (field, column, orderBy = `${column} ASC`, additionalWhere = '', limit = null) => {
      ...
      if (contextSubquery !== null) {
        finalWhere = `${where} AND id IN (${contextSubquery})`;
        finalParams = [...params, ...contextParams];
      }
      ...
      return this.db.prepare(query).all(...finalParams);
    };
    ...
    if (contextSubquery !== null) {
      totalWhere = `${where} AND id IN (${contextSubquery})`;
      totalParams = [...params, ...contextParams];
    }
    ...
    const getTimeBuckets = () => {
      ...
      if (contextSubquery !== null) {
        tbWhere = `${where} AND id IN (${contextSubquery})`;
        tbParams = [...params, ...contextParams];
      }
      ...
      return this.db.prepare(query).all(...tbParams);
    };
