  ...
  buildWhereClause(filters, excludeField = null) {
    ...
    const addFilterClause = (field, dbColumn, includeList, excludeList) => {
      ...
      if (includeList.length > 0) {
        if (nonNullInclude.length > 0 && includeNulls) {
          where += ` AND (${dbColumn} IN (${nonNullInclude.map(() => '?').join(',')}) OR ${dbColumn} IS NULL)`;
          params.push(...nonNullInclude);
        } else if (nonNullInclude.length > 0) {
          where += ` AND ${dbColumn} IN (${nonNullInclude.map(() => '?').join(',')})`;
          params.push(...nonNullInclude);
        } else if (includeNulls) {
          where += ` AND ${dbColumn} IS NULL`;
        }
      }

      // Add exclude clause
      if (excludeList.length > 0) {
        if (nonNullExclude.length > 0 && excludeNulls) {
          where += ` AND (${dbColumn} NOT IN (${nonNullExclude.map(() => '?').join(',')}) AND ${dbColumn} IS NOT NULL)`;
          params.push(...nonNullExclude);
        } else if (nonNullExclude.length > 0) {
          where += ` AND ${dbColumn} NOT IN (${nonNullExclude.map(() => '?').join(',')})`;
          params.push(...nonNullExclude);
