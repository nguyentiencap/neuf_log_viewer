/**
 * Pipeline Module
 * Provides dynamic filter suggestions for the filter pipeline feature.
...
class PipelineService {
  /**
   * Generate pipeline suggestions based on current data.
  ...
  static getSuggestions(filterOptions = {}, currentFilters = {}) {
    ...
    if (devices.length > 1 && !(currentFilters.deviceInclude && currentFilters.deviceInclude.length)) {
      ...
      suggestions.push({
        ...
        description: `Show only logs from device "${topDevice.device_id}" (${Number(topDevice.count).toLocaleString()} entries, most active)`,
    ...
    if (components.length >= 5 && !(currentFilters.componentExclude && currentFilters.componentExclude.length)) {
      ...
      if (topComponents.length > 0) {
        suggestions.push({
          ...
          description: `Exclude the ${topComponents.length} most frequent components to reduce noise`,
    ...
    // ── Thread suggestions ────────────────────────────────────────────────────
    if (threads.length >= 5 && !(currentFilters.threadExclude && currentFilters.threadExclude.length)) {
      const topN = Math.min(5, threads.length);
      const topThreads = threads.slice(0, topN).map(t => t.thread_name).filter(Boolean);
      if (topThreads.length > 0) {
        const preview = topThreads.slice(0, 3).join(', ');
        const suffix = topThreads.length > 3 ? ', ...' : '';
        suggestions.push({
          id: 'exclude_top_threads',
          label: `🧵 Exclude top ${topThreads.length} noisiest threads`,
          description: `Exclude: ${preview}${suffix}`,
          filters: { threadExclude: topThreads }
        });
      }
    }
